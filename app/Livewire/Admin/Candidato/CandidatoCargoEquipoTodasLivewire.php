...
class CandidatoCargoEquipoTodasLivewire extends Component
{
    use WithPagination;
    ...
    /*protected $listeners = [
        'integranteEquipoAgregado' => '$refresh',
    ];*/
    ...
    public function removeIntegrante(int $integranteId)
    {
        ...
        CandidatoCargoEquipo::where('lider_candidato_cargo_id', $this->liderId)
            ->where('integrante_candidato_cargo_id', $integranteId)
            ->delete();

        $this->dispatch('alertaLivewire', "Eliminado");
        
        //$this->dispatch('integranteEquipoQuitado', $this->liderId);

        // refrescar la relación del líder (opcional)
        $this->lider->load('equipo.integrante.candidato', 'equipo.integrante.cargo');

        // reset page para evitar páginas vacías
        $this->resetPage();
    }
    ...
    {
        $query = $this->lider
            ->equipo() // devuelve CandidatoCargoEquipo query
            ->with(['integrante.candidato', 'integrante.cargo', 'integrante.partido']);

        // filtros que deben aplicarse sobre la relación integrante
        if ($this->filtrarPorPartido && $this->lider->partido_id) {
            $query->whereHas('integrante', fn($q) => $q->where('partido_id', $this->lider->partido_id));
        }

        if ($this->filtrarPorAlianza && $this->lider->alianza_id) {
            $query->whereHas('integrante', fn($q) => $q->where('alianza_id', $this->lider->alianza_id));
        }

        if (!empty($this->nivel_id)) {
            $query->whereHas('integrante', fn($q) => $q->where('nivel_id', $this->nivel_id));
        }

        if (!empty($this->cargo_id)) {
            $query->whereHas('integrante', fn($q) => $q->where('cargo_id', (int) $this->cargo_id));
        }

        if (!empty($this->pais_id)) {
            $query->whereHas('integrante', fn($q) => $q->where('pais_id', $this->pais_id));
        }

        if (!empty($this->region_id)) {
            $query->whereHas('integrante', fn($q) => $q->where('region_id', $this->region_id));
        }

        if (!empty($this->provincia_id)) {
            $query->whereHas('integrante', fn($q) => $q->where('provincia_id', $this->provincia_id));
        }

        if (!empty($this->distrito_id)) {
            $query->whereHas('integrante', fn($q) => $q->where('distrito_id', $this->distrito_id));
        }

        if (!empty($this->buscar)) {
            // búsqueda por nombre del candidato (tabla relacionada)
            $query->whereHas('integrante.candidato', fn($q) => $q->where('nombre', 'like', '%' . $this->buscar . '%'));
        }

        $seleccionados = $query->orderBy('created_at', 'desc')->paginate($this->perPage);

        return view('livewire.admin.candidato.candidato-cargo-equipo-todas-livewire', compact('seleccionados'));
    }
}
